...
fn merge_sort(list: &mut [i32], mut first_len: usize) {
    ...
    unsafe {
        ...
        // 找到 first[first_len-1] 元素在 second 中的位置
        // 那么 second 中 second_len 前的元素都小于 first[first_len-1]，
        // 之后的元素都大于 first[first_len-1]
        second_len = gallop_left(first.get_unchecked(first_len - 1), second, Mode::Reverse);
        if 0 == second_len {
            return;
        }

        // 找到 second[0] 元素在 first 中的位置
        // 那么 first 中 first_off 前的元素都小于等于 second[0]
        // 之后的元素都大于 second[0]
    ...
    let new_list = list
        .split_at_mut(first_off).1
        .split_at_mut(first_len + second_len).0;
    if first_len > second_len {
        // B 和 C 合并，借助 temp 从 new_list 末尾开始合并
        merge_hi(new_list, first_len, second_len);
    } else {
        // B 和 A 合并，借助 temp 从 new_list 首部开始合并
...
impl<'a> MergeLo<'a> {
    unsafe fn new(list: &'a mut [i32], first_len: usize) -> Self {
        let mut ret_val = MergeLo {
            list_len: list.len(),
            first_pos: 0,
            first_len: first_len,  // run1 和 run2 挨着，所以 run2 起始位置就等于 run1 长度
            second_pos: first_len, // 排序结果写回原始集合，且从 run1 的起始位置开始写
            dest_pos: 0,
            list: list,
...
impl<'a> MergeHi<'a> {
    ...
    unsafe fn merge(&mut self) {
        ...
        while self.first_pos < self.dest_pos && self.first_pos >= 0 {
            ...
            if (second_count | first_count) < MIN_GALLOP {
                if self.temp.get_unchecked(self.second_pos as usize)
                ...
                if self.first_pos < self.dest_pos && self.first_pos >= 0 {
                    second_count = self.dest_pos as usize + 1
                        - gallop_left(
                            self.list.get_unchecked(self.first_pos as usize),
...
fn main() {
    let mut nums: Vec<i32> = vec![
        ...
        18, 17, 15, 11,  9, -1,  5,  6,  1,  3, 21, 22,
        40, 39, 38, 37, 36, 35, 34, 33, 32, 31, 30, 24,
